...
class SaleStatisticsView(APIView):
    def get(self, request, *args, **kwargs):
        user = request.user
        total_revenue_for_user = user.sale_set.aggregate(total_revenue=Sum('revenue'))['total_revenue']
        ...
        total_revenue = Sale.objects.aggregate(
            total_revenue=Sum('revenue'))['total_revenue']
        total_sales = Sale.objects.count()
        average_sale_all_user = total_revenue / total_sales

        highest_revenue_sale_for_current_user = user.sale_set.order_by('-revenue').first()
